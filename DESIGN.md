...
更新日：2026-09-11（ソースはIDのみ、関係と知識MDはLogos側に集約）

状態：初期実装の基準となる設計。現在の実装範囲と確認結果は [確認記録](docs/VALIDATION.md)、操作方法は [利用手順](docs/USAGE.md) を参照。
...
Markdownには概念、知識、設計意図、判断基準などの説明を書く。プログラムの判定・処理は通常のソースコードに実装する。知識のMDはLogos内に集めてオントロジー注釈を付ける。ソースには `// @logos-id ID` だけを置き、読取器が現在のコードと位置を取得する。

たとえば、Markdownの章に「公開APIの項目を削除しない」という基準を記述し、MDの基準から実装関数のIDへ関係を付ける。本文と関係はMD、コードとIDはソースから読み取る。判定ロジックをMarkdownへ移したり、ソースの内容を別の知識ファイルに複製したりする必要はない。
...
| 実装 | 関数・クラスなどのソースコード | ID、コード、原本の位置。種類と宣言名は読取器が派生させる |
...
さらに、MDの基準から実装IDへの `implemented_by`、MDからテストIDへの `verified_by`、MDで記述する要素間の `depends_on`、情報の由来を示す `derived_from` などを共通の関係として保持する。関係はMDに一度記述すれば、ソースのIDからも逆引きできる。ソースからMDや他ソースへの関係注釈は持たない。依存先の基準を無条件に継承するなど、異なる関係の意味を混ぜない。
...
MDから実装IDへの参照と、ホスト・拡張の登録情報を結び付けることで、エージェントは「何ができるか」から「どう呼ぶか」「何のためか」「どこを変えて確かめるか」へ辿れる。実行可能状態の正本は登録情報とし、注釈に有効・無効の状態を重複して書かない。
...
knowledge/shared/          共有知識
knowledge/projects/        各プロジェクトの知識（Logos内で管理）
...
プロジェクトはIDとローカルの作業場所で接続する。プロジェクト内のソースの読込範囲を登録し、Logos内に保存した固有知識と共有知識を併せて扱う。他リポジトリのMarkdownは知識の索引対象にせず、知識の追加・編集操作も接続先へ書き込まない。対象プロジェクトのコードや資料をLogosへ丸ごと複製する必要はない。同じフォルダがLogos自身の実装と知識を持っていてもよい。
