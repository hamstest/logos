...
if ((await host.call('knowledge.get', { id: 'concept/project-inspection' }, ctx)).status !== 'available') {
  await host.call('knowledge.create', { rootId: project + '/knowledge', path: 'inspection.md', annotation: { format: 1, id: 'concept/project-inspection', kind: 'concept', title: 'Project inspection' }, body: '# Project inspection\nRead and describe the selected project.' }, ctx);
}
...
await host.call('improvement.learn', { taskId: task.id, rootId: project + '/knowledge', path: task.id.slice(5) + '-lesson.md', id: lessonId, title: 'Inspect the selected directory', body: 'Use the connected project directory as the working directory when collecting its entries. This run verified the selected directory, not the behavior of an autonomous agent.', concepts: ['concept/project-inspection'], scope: project }, ctx);
...
assert.ok(criterion.some(edge => edge.target === 'implementation/public-documentation'));
...
assert.ok(implementation.relations.some(edge => edge.source === 'criterion/public-documentation'));
