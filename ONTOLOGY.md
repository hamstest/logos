...
更新日：2026-09-11（ソースはIDのみ、関係はLogosのMDへ集約）

状態：初期実装の注釈仕様。MarkdownとTypeScript/JavaScriptの読取器を実装済み。[全体設計](DESIGN.md) の情報モデルと原本の扱いを具体化する。

## 1. 何を原本にするか

文章で説明するものはMarkdown、プログラムは実装のソースコードを原本にする。知識のMarkdownはLogos内で管理し、意味と関係を記述する。ソース側には参照先のIDコメントだけを置き、MDからそのIDへ接地する。

判断基準の説明は、その意味、適用条件、理由、確認したい内容を文章で書く。機械的に判定する実装があれば、ソースの関数やクラスにIDを付け、MD側の基準からそのIDを参照する。文章だけで利用する基準があってもよい。

MDのオントロジー注釈は対象の識別・意味・関係を記述するYAMLデータとする。ソースにはこのデータを埋め込まない。条件分岐や計算は通常の実装コードに書く。Markdownの注釈を実行コードへ変換する仕組みや、判定ロジックを注釈用の条件式へ翻訳する仕組みは作らない。
...
この表の項目はMD用である。ソースからはIDだけを読み、`kind: source`、宣言名、接続元のプロジェクト範囲を派生させる。ソースノード自身の `links` は常に空であり、関係はMDからの参照を逆引きする。

読取器は、注釈やIDに加えて以下を共通ビューへ載せる。
...
ここでの `attach` はMarkdown内の対象範囲を決める項目である。知識の共有範囲である `scope`、意味上の適用対象である `applies_to` とは別に扱う。
...
TypeScript/JavaScriptでは、独立した行に `// @logos-id ID` を置く。ID以外の種類・範囲・関係・版などは書かない。ソースに適用するプロジェクト範囲は接続設定から取得する。

IDは直後の関数、クラス、メソッド、一つの変数を定義する宣言、または式文へ結び付ける。式文に対応するため、テスト群の `describe(...)` もひとまとまりで参照できる。間に置けるのは空白と通常の説明コメントだけであり、未対応の宣言を飛び越えて結び付けない。`export` や修飾子は宣言に含む。

後置コメント、同じ対象への複数ID、IDの後に余分な項目があるコメントは診断を返す。ソース用の `attach: file` は持たず、ファイル全体を注釈メタデータで指定する方式は廃止する。

コメントと対象範囲は構文解析で取得する。文字列やテンプレート内の見本は登録しない。行番号は取得した位置情報であり、参照の識別には使わない。読取のためにソースをimport・実行しない。

旧 `/* @logos ... */` ブロックは `legacy-source-annotation` として知らせ、そこから意味や関係を読み込まない。移行時はIDを保ったまま短いコメントに替え、旧 `implements` はMD側からの `implemented_by`、旧 `verifies` は `verified_by` へ書き換える。ソース間の関係を表す専用注釈は廃止する。

他言語への対応は、IDコメントの抽出と対象範囲を返す読取器の追加として行う。共通ノードと関係を使う側は言語ごとの構文に依存させない。
...
links:
  ...
  - relation: implemented_by
    target: implementation/check-public-members
```
## 公開項目を削除しない
...
ソース側には、この基準を判定する実際の処理とIDだけを置く。

```typescript
// @logos-id implementation/check-public-members
...
関係はLogosのMD側に一度だけ書く。ソースにはMDへのリンクや、ソース同士の関係を書かない。共通の索引は、関数から基準を取得する方向と、基準から実装を探す方向の両方を提供する。説明とコードを重複させる必要はない。

基準と実装は一対一に限定しない。一つの基準に複数の実装があっても、一つの実装が複数の基準に関わっていてもよい。`implemented_by` は記述された対応を示し、すべての基準を満たすことの証明にはしない。
...
| `implemented_by` | MDの基準・仕様・機能 → ソースの実装ID | 意味とコードを双方向に辿る |
| `verified_by` | MDの基準・仕様 → ソースのテストID | 変更時に、確認方法を発見する |
| `depends_on` | MDで記述する要素 → MDの依存先 | 依存先と影響を受け得る要素を発見する |
...
1. Logos内の知識ルートからMarkdown、接続プロジェクトのソースルートから対応言語のコードを取得する。他プロジェクトのMarkdownは知識として索引化しない。
2. MD読取器は注釈と本文、ソース読取器はIDと対象コードの現在位置を取り出し、共通ノードへ変換する。
...
プロジェクト範囲はノードの取得と関係の探索の両方で適用する。共有された基準から実装を探した場合も、別プロジェクトの固有情報を通常の文脈へ混ぜない。範囲外の情報と、読込範囲内の参照切れを区別する。
...
入力・出力とハンドラは操作登録側で定め、注釈に同じ呼出定義を重複させない。MDの注釈は意味の関係、ソースコメントはID、登録は実際の呼出に必要な対応を持つ。操作一覧に実装IDを示し、知識取得から原本と登録操作を照合できるようにする。MDの参照先が解決できない場合は診断を返す。
...
2. 二つの隣り合う関数に注釈を置き、文字列内の `@logos-id` や説明用のコード例を誤って登録しない。
...
後置注釈、式文以外の細かな構文要素へのID、任意のシンボルを指定する構文、多言語の同時対応、差分索引は、この最初の例が動いてから必要に応じて追加する。最初から完全なコード解析基盤を目指さず、対象範囲と原本への対応を確実に扱う。
