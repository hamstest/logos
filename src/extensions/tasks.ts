...
export const tasks: Extension = {
  ...
  setup(host) {
    ...
    host.register({ name: 'task.create', description: 'Start a task record; knowledge access does not require one.', input: z.object({ objective: z.string().min(1), project: z.string().optional(), references: z.array(z.string()).default([]), improvement: taskSchema.shape.improvement }), output: taskResultSchema, async handler(i, ctx) {
    ...
    host.register({ name: 'task.delete', description: 'Move a non-running task record to recoverable trash; references are not cascaded.', input: z.object({ id: z.string(), expectedHash: z.string() }), output: recordSchema, async handler(i, ctx) {
