...
export const rootSchema = z.object({ id: idSchema, path: z.string(), scope: idSchema, include: z.array(z.string()).default(['.']), role: z.enum(['knowledge', 'source']).default('knowledge') });
export const projectSchema = z.object({ id: idSchema.refine(id => id !== 'shared', 'shared is reserved for shared knowledge'), path: z.string(), title: z.string(), sources: z.array(z.string()).default(['.']) });
...
// @logos-id configuration
export async function projects(workspace: string): Promise<Project[]> {
  const self: Project = { id: 'logos', path: workspace, title: 'Logos', sources: [] };
...
// @logos-id source-roots

// The flat project ID is the directory name, independent of the external checkout.
export async function projectKnowledgeRoot(workspace: string, project: string): Promise<SourceRoot> {
  ...
  return { id: project + '-knowledge', path: await containedPath(workspace, 'knowledge/projects/' + project), scope: project, include: ['.'], role: 'knowledge', optional: true };
}
